  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        setIntroText(recipe.intro_text || "");
        setNotes(recipe.notes || "");
        setFeaturedImage(recipe.featured_image_url || "");
        ...
        // Set video links
        setYoutubeUrl(recipe.video_links?.youtube || "");
        setInstagramUrl(recipe.video_links?.instagram || "");
        setTiktokUrl(recipe.video_links?.tiktok || "");
        ...
        if (recipe.recipe_categories) {
          setSelectedCategories(
            recipe.recipe_categories.map((rc: any) => rc.category_id)
          );
        }
      } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/recipes", {
        ...
        body: JSON.stringify({ ...recipeData, id: recipeId }),
      });
    ...
    <div className="max-w-5xl">
      ...
      {success && (
        <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg">
          <p className="text-sm text-green-600">✓ Recipe updated successfully!</p>
        </div>
      )}
